...
class DateTimePicker extends React.Component{
	...
	valid = (current) => {
		if(this.props.birth)
			return current.isBefore(moment().subtract(10, 'years'));  // 생년월일 설정은 현재로부터 10년전 날짜만 선택가능
		else
			return current.isAfter(moment().subtract(1, 'days'));	// 그외의 경우는 오늘 이후의 날짜만 선택 가능
	};
	render(){
		const { id, value, label, viewMode, birth, shrink, onChange, placeholder } = this.props;
		...
		<div>
			<FormControl style={{width: "260px"}}>
				...
				<Datetime
					...
					inputProps={{ placeholder: placeholder, style:{fontFamily: "Godo", fontSize:"1em", paddingLeft:"5px"} }}
					locale="en"
				/>
			</FormControl>
		</div>
